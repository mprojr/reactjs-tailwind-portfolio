        ...
        {projects.map(project => (
          <div key={project.id} className='relative border border-spacing-1 h-[200px] transparent rounded-xl overflow-hidden'>
            <Link to={project.detailsPage} className='fill-transparent w-full h-full group2'>
              ...
              <div className='absolute w-full top-[0rem] left-1/2 transform -translate-x-1/2 bg-black bg-opacity-40 text-center text-white text-lg font-bold opacity-0 group2-hover:opacity-100 delay-200 duration-200 rounded-2xl'>
                {project.title}
              </div>
            </Link>
          </div>
